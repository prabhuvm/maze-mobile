...
const UpdateProfileScreen = () => {
  ...
  const [unameStatusMessage, setUNameStatusMessage] = useState('');
  ...
  const pencil_img = '../../assets/icons/pencil.png';
  ...
  const uploadPhoto = async (photo) => {
    ...
    try {
      const accessToken = await AsyncStorage.getItem('accessToken');
      await apiClient.post(`/users/${username}/user_pic/`, formData, {
        headers: {
  ...
  const saveUsername = async () => {
    const accessToken = await AsyncStorage.getItem('accessToken');
    await apiClient
      .post(`/users/${username}/username/`, { username: newUsername }, {
        headers: {
          Authorization: `Bearer ${accessToken}`,
        },
      })
      .then((response) => {
        if (response.data.status == 1) {
          ...
          setUNameStatusMessage('Username set successfully!');
          setUsernameError(false);
        } else {
          setUsernameError(true);
          setUsernameErrMessage(response.data.detail);
        }
      })
      .catch((error) => {
        setUsernameError(true);
        setUsernameErrMessage('Error while setting username. Please try again.');
        console.error('Error sending verification code:', error);
      });
  };
        ...
        <View style={styles.fieldContainer}>
          <View style={styles.labelContainer}>
            ...
            <TouchableOpacity onPress={() => handleFieldEdit('username', nameInputRef)}>
              <Image source={require(pencil_img)} style={styles.icon} />
            </TouchableOpacity>
          </View>
          <View style={styles.emailContainer}>
            ...
            />
            <TouchableOpacity style={styles.verifyButton} onPress={saveUsername}>
              <Text style={styles.verifyButtonText}>Save</Text>
            </TouchableOpacity>
          </View>
          {!usernameError && <Text style={styles.statusMessage}>{unameStatusMessage}</Text>}
          {usernameError && <Text style={styles.verifyErrMessage}>{usernameErrMessage}</Text>}
        </View>
      </View>
        ...
        <View style={styles.fieldContainer}>
          <View style={styles.labelContainer}>
            ...
            <View style={styles.iconContainer}>
              {verified ? (
                <Image source={require('../../assets/icons/pos-tick.png')} style={styles.icon} />
              ) : (
                <Image source={require('../../assets/icons/neg-tick.png')} style={styles.icon} />
              )}
